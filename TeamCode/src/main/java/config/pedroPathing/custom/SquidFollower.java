package config.pedroPathing.custom;
...
import config.helpers.logging.DownsampledWriter;
import config.helpers.messages.PoseMessage;
...
public class SquidFollower {
  ...
  private SquidController secondaryTranslationalPIDF;
  private SquidController secondaryTranslationalIntegral;
  private SquidController translationalPIDF;
  private SquidController translationalIntegral;
  private SquidController secondaryHeadingPIDF;
  private SquidController headingPIDF;
  private FilteredSquidController secondaryDrivePIDF;
  private FilteredSquidController drivePIDF;
  ...
  /**
   * This initializes the follower. In this, the DriveVectorScaler and PoseUpdater is instantiated,
   * the drive motors are initialized and their behavior is set, and the variables involved in
   * approximating first and second derivatives for teleop are set.
   */
  ...
  /**
   * This sets the current pose, using offsets so no reset time delay. This is better than the Road
   * Runner reset, in general. Think of using offsets as setting trim in an aircraft. This can be
   * reset as well, so beware of using the resetOffset() method.
   *
  ...
  /**
   * This follows a PathChain. Drive vector projection is only done on the last Path. This also
   * makes the Follower hold the last Point on the PathChain.
   *
    ...
    if (!teleopDrive) {
      if (currentPath != null) {
        if (holdingPosition) {
          ...
          if (isBusy) {
            closestPose =
                currentPath.getClosestPoint(poseUpdater.getPose(), BEZIER_CURVE_SEARCH_LIMIT);
          ...
          if (poseUpdater.getVelocity().getMagnitude() < 1.0
              && currentPath.getClosestPointTValue() > 0.8
          ...
          if (currentPath.isAtParametricEnd()
            ...
            if (followingPathChain && chainIndex < currentPathChain.size() - 1) {
              ...
              if ((System.currentTimeMillis() - reachedParametricPathEndTime
                ...
                if (holdPositionAtEnd) {
                  ...
                  holdPoint(
                      new BezierPoint(currentPath.getLastControlPoint()),
                      currentPath.getHeadingGoal(1));
                } else {
                  if (logDebug && isBusy) {
                    Log.d(
                        "Follower_final_logger::",
                        "isAtParametricEnd:"
                            ...
                            + getPose()
                            + " | t-value: "
                            + String.format("%3.5f", currentPath.getClosestPointTValue())
                            + " | velocity: "
                            + String.format("%3.2f", poseUpdater.getVelocity().getMagnitude())
                            + " | distance: "
                            + String.format(
                                "%3.2f", MathFunctions.distance(poseUpdater.getPose(), closestPose))
                            + " | heading (degree): "
                            + String.format(
                                "%3.2f",
                                Math.toDegrees(
      ...
      estimatedPoseWriter.write(new PoseMessage(poseUpdater.getPose()));
      targetPoseWriter.write(new PoseMessage(closestPose));
    } else {
   ...
   *
   * @param forwardDrive determines the forward drive vector for the robot in teleop. In field
   *     centric movement, this is the x-axis.
   * @param lateralDrive determines the lateral drive vector for the robot in teleop. In field
   *     centric movement, this is the y-axis.
   ...
   */
  public void setTeleOpMovementVectors(
      double forwardDrive, double lateralDrive, double heading, boolean robotCentric) {
  ...
  /**
   * This calculates an averaged approximate velocity and acceleration. This is used for a real-time
   * correction of centripetal force, which is used in teleop.
   */
  public void calculateAveragedVelocityAndAcceleration() {
    ...
    averageVelocity =
        MathFunctions.scalarMultiplyVector(averageVelocity, 1.0 / ((double) velocities.size() / 2));
    ...
    averagePreviousVelocity =
        MathFunctions.scalarMultiplyVector(
            averagePreviousVelocity, 1.0 / ((double) velocities.size() / 2));
    ...
    averageAcceleration =
        MathFunctions.scalarMultiplyVector(averageAcceleration, 1.0 / accelerations.size());
  }
    ...
    for (PathCallback callback : currentPathChain.getCallbacks()) {
      if (!callback.hasBeenRun()) {
        if (callback.getType() == PathCallback.PARAMETRIC) {
          ...
          if (chainIndex >= callback.getIndex()
              && System.currentTimeMillis() - pathStartTimes[callback.getIndex()]
   ...
   *
   * <p>Note: This vector is clamped to be at most 1 in magnitude.
   *
  ...
  public Vector getDriveVector() {
    ...
    if (followingPathChain && chainIndex < currentPathChain.size() - 1) {
      return new Vector(
          driveVectorScaler.getMaxPowerScaling(),
    ...
    if (Math.abs(driveError) < drivePIDFSwitch && useSecondaryDrivePID) {
      ...
      driveVector =
          new Vector(
              MathFunctions.clamp(
                  secondaryDrivePIDF.runPIDF()
                      + secondaryDrivePIDFFeedForward * MathFunctions.getSign(driveError),
                  -driveVectorScaler.getMaxPowerScaling(),
                  driveVectorScaler.getMaxPowerScaling()),
              currentPath.getClosestPointTangentVector().getTheta());
    ...
    driveVector =
        new Vector(
            MathFunctions.clamp(
                drivePIDF.runPIDF() + drivePIDFFeedForward * MathFunctions.getSign(driveError),
                -driveVectorScaler.getMaxPowerScaling(),
                driveVectorScaler.getMaxPowerScaling()),
            currentPath.getClosestPointTangentVector().getTheta());
    return MathFunctions.copyVector(driveVector);
  ...
  /**
   * This returns the velocity the robot needs to be at to make it to the end of the Path at some
   * specified deceleration (well technically just some negative acceleration).
   *
  ...
  public double getDriveVelocityError() {
    ...
    double forwardDistanceToGoal =
        MathFunctions.dotProduct(forwardHeadingVector, distanceToGoalVector);
    double forwardVelocityGoal =
            ...
            * Math.sqrt(
                Math.abs(
                    -2
                        ...
                        * (forwardDistanceToGoal <= 0 ? 1 : -1)
                        * forwardDistanceToGoal));
    double forwardVelocityZeroPowerDecay =
        forwardVelocity
            - MathFunctions.getSign(forwardDistanceToGoal)
                * Math.sqrt(
                    Math.abs(
                        Math.pow(forwardVelocity, 2)
                            + 2 * forwardZeroPowerAcceleration * Math.abs(forwardDistanceToGoal)));
    ...
    double lateralDistanceToGoal =
        MathFunctions.dotProduct(lateralHeadingVector, distanceToGoalVector);

    double lateralVelocityGoal =
            ...
            * Math.sqrt(
                Math.abs(
                    -2
                        ...
                        * (lateralDistanceToGoal <= 0 ? 1 : -1)
                        * lateralDistanceToGoal));
    double lateralVelocityZeroPowerDecay =
        lateralVelocity
            - MathFunctions.getSign(lateralDistanceToGoal)
                * Math.sqrt(
                    Math.abs(
                        Math.pow(lateralVelocity, 2)
                            + 2 * lateralZeroPowerAcceleration * Math.abs(lateralDistanceToGoal)));

    Vector forwardVelocityError =
        new Vector(
            forwardVelocityGoal - forwardVelocityZeroPowerDecay - forwardVelocity,
            forwardHeadingVector.getTheta());
    Vector lateralVelocityError =
        new Vector(
            lateralVelocityGoal - lateralVelocityZeroPowerDecay - lateralVelocity,
            lateralHeadingVector.getTheta());
    Vector velocityErrorVector =
        MathFunctions.addVectors(forwardVelocityError, lateralVelocityError);

    previousRawDriveError = rawDriveError;
    rawDriveError =
        velocityErrorVector.getMagnitude()
            * MathFunctions.getSign(
                MathFunctions.dotProduct(
                    velocityErrorVector, currentPath.getClosestPointTangentVector()));
  ...
  /**
   * This returns a Vector in the direction of the robot that contains the heading correction as its
   * magnitude. Positive heading correction turns the robot counter-clockwise, and negative heading
   * correction values turn the robot clockwise. So basically, Pedro Pathing uses a right- handed
   * coordinate system.
   *
   * <p>Note: This vector is clamped to be at most 1 in magnitude.
   *
  ...
  public Vector getHeadingVector() {
    ...
    headingError =
        MathFunctions.getTurnDirection(
                poseUpdater.getPose().getHeading(), currentPath.getClosestPointHeadingGoal())
            * MathFunctions.getSmallestAngleDifference(
                poseUpdater.getPose().getHeading(), currentPath.getClosestPointHeadingGoal());
  ...
  public Vector getCorrectiveVector() {
    ...
    if (corrective.getMagnitude() > driveVectorScaler.getMaxPowerScaling()) {
      return MathFunctions.addVectors(
          centripetal,
          MathFunctions.scalarMultiplyVector(
              translational, driveVectorScaler.findNormalizingScaling(centripetal, translational)));
    }
  ...
  public Vector getTranslationalCorrection() {
    ...
    if (MathFunctions.distance(poseUpdater.getPose(), closestPose) < translationalPIDFSwitch
      ...
      secondaryTranslationalIntegralVector =
          MathFunctions.addVectors(
              secondaryTranslationalIntegralVector,
              new Vector(
                  secondaryTranslationalIntegral.runPIDF() - previousSecondaryTranslationalIntegral,
      ...
      translationalVector.setMagnitude(
          secondaryTranslationalPIDF.runPIDF() + secondaryTranslationalPIDFFeedForward);
      translationalVector =
          MathFunctions.addVectors(translationalVector, secondaryTranslationalIntegralVector);
    } else {
      ...
      translationalIntegralVector =
          MathFunctions.addVectors(
              translationalIntegralVector,
              new Vector(
                  translationalIntegral.runPIDF() - previousTranslationalIntegral,
                  translationalVector.getTheta()));
      ...
      translationalVector =
          MathFunctions.addVectors(translationalVector, translationalIntegralVector);
    }

    translationalVector.setMagnitude(
        MathFunctions.clamp(
            translationalVector.getMagnitude(), 0, driveVectorScaler.getMaxPowerScaling()));
   ...
   *
   * <p>Note: This vector is clamped to be between [0, 1] in magnitude.
   *
                    ...
                    * Math.pow(
                        MathFunctions.dotProduct(
                            ...
                            MathFunctions.normalizeVector(
                                currentPath.getClosestPointTangentVector())),
                        2)
                    * curvature,
                -driveVectorScaler.getMaxPowerScaling(),
                driveVectorScaler.getMaxPowerScaling()),
  ...
  /**
   * This returns whether the follower is at the parametric end of its current Path. The parametric
   * end is determined by if the closest Point t-value is greater than some specified end t-value.
  ...
  /**
   * This returns the t value of the closest point on the current Path to the robot In the absence
   * of a current Path, it returns 1.0.
   *
  ...
  private void debugLog() {
    Log.d(
        ...
        "isAtParametricEnd:"
            ...
            + String.format("%3.2f", poseUpdater.getVelocity().getMagnitude())
            + " | distance: "
            + String.format("%3.2f", MathFunctions.distance(poseUpdater.getPose(), closestPose))
            + " | heading (degree): "
            + String.format(
                "%3.2f",
                Math.toDegrees(
  ...
  /**
   * This will update the PIDF coefficients for primary Heading PIDF mid run can be used between
   * paths
   *
  ...
  /**
   * This will update the PIDF coefficients for primary Translational PIDF mid run can be used
   * between paths
   *
   * @param set PIDF coefficients you would like to set.
   */
  ...
  /**
   * This will update the PIDF coefficients for primary Drive PIDF mid run can be used between paths
   *
   * @param set PIDF coefficients you would like to set.
   */
  ...
  /**
   * This will update the PIDF coefficients for secondary Heading PIDF mid run can be used between
   * paths
   *
   * @param set PIDF coefficients you would like to set.
   */
  ...
  /**
   * This will update the PIDF coefficients for secondary Translational PIDF mid run can be used
   * between paths
   *
   * @param set PIDF coefficients you would like to set.
   */
  ...
  /**
   * This will update the PIDF coefficients for secondary Drive PIDF mid run can be used between
   * paths
   *
   * @param set PIDF coefficients you would like to set.
   */
  ...
  public boolean atPoint(Point point, double xTolerance, double yTolerance) {
    return Math.abs(point.getX() - getPose().getX()) < xTolerance
        && Math.abs(point.getY() - getPose().getY()) < yTolerance;
  }
