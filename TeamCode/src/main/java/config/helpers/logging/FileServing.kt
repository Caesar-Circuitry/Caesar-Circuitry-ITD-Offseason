package config.helpers.logging
...
private fun newStaticAssetHandler(
...
private fun registerAssetsUnderPath(
...
private fun newStaticFileHandler(f: File): WebHandler {
    return WebHandler { session: IHTTPSession ->
        if (session.method == NanoHTTPD.Method.GET) {
            ...
            return@WebHandler NanoHTTPD.newChunkedResponse(
                NanoHTTPD.Response.Status.OK,
                mimeType,
                FileInputStream(f),
            )
...
object TuningFiles {
    ...
    private var whm: WebHandlerManager? = // guarded by ioLock
        null
        ...
        manager.register("/logs/download") { session: IHTTPSession ->
            ...
            if (pairs.size != 1) {
                return@register NanoHTTPD.newFixedLengthResponse(
                    ...
                    NanoHTTPD.MIME_PLAINTEXT,
                    "expected one query parameter, got " + pairs.size,
                )
            }
